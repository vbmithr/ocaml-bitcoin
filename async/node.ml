...
let my_addresses = BitcoinAddr.of_string_exn "mjVrE2kfz42sLR5gFcfvG6PwbAjhpmsKnn"
...
let request_hdrs w start =
  let msg = Message.GetHeaders (GetHashes.create [ start ]) in
...
let process_msg w msg =
  ...
  | Reject rej -> error "%s" (Format.asprintf "%a" Reject.pp rej)
  | SendHeaders -> debug "Got SendHeaders!"
  ...
  | SendCmpct t -> sexp ~level:`Debug (SendCmpct.sexp_of_t t)
  | GetAddr -> debug "Got GetAddr!"
  | Addr _ -> debug "Got Addr!"
  | Ping i ->
    debug "Got Ping!";
    let cs = Message.to_cstruct ~network:!network buf (Pong i) in
    write_cstruct2 w buf cs;
    debug "Sent Pong"
  | Pong _ -> debug "Got Pong!"
  | GetBlocks _ -> debug "Got GetBlocks!"
  | GetData _ -> debug "Got GetData!"
  | GetHeaders _ -> debug "Got GetHeaders!"
  | Block _ -> debug "Got Block!"
  | MerkleBlock mblock ->
    debug "MerkleBlock %s" (Sexplib.Sexp.to_string_hum (MerkleBlock.sexp_of_t mblock))
  | Headers hdrs ->
    List.iteri hdrs ~f:(fun _i h ->
    ...
    if List.length hdrs = 2000 then request_hdrs w !best_hh
  | Inv invs ->
    List.iter invs ~f:(fun inv ->
      debug "Inv %s" (Sexplib.Sexp.to_string_hum (Inv.sexp_of_t inv)))
  | NotFound _ -> debug "Got NotFound!"
  | MemPool -> debug "Got MemPool!"
  | Tx _ ->
    debug "Got Tx!"
    (* debug "%s" (Sexplib.Sexp.to_string_hum (Transaction.sexp_of_t tx)) *)
  | FeeFilter fee -> debug "Got FeeFilter: %Ld" fee
  | FilterAdd _ -> debug "Got FilterAdd!"
  | FilterClear -> debug "Got FilterClear!"
  | FilterLoad _ -> debug "Got FilterLoad!"
;;

let handle_chunk w buf ~pos ~len =
  (* debug "consume_cs %d %d" pos len ; *)
  if len < MessageHeader.size
  then return (`Consumed (0, `Need MessageHeader.size))
  else (
    ...
    if Cstruct.length cs_payload < hdr.size
    then return (`Consumed (0, `Need msg_size))
    else (
...
let main_loop port _s r w =
  ...
  let cs =
    Message.to_cstruct
      ~network:!network
      buf
      (Version (Version.create ~recv_port:port ~trans_port:port ()))
  in
  write_cstruct w (Cstruct.sub buf 0 cs.off);
  Reader.read_one_chunk_at_a_time r ~handle_chunk:(handle_chunk w)
  >>= function
  | `Eof ->
    info "EOF";
    Deferred.unit
...
let main testnet host port _daemon _datadir _rundir _logdir loglevel () =
  ...
  let host =
    match testnet, host with
    | _, Some host -> host
    | true, None -> List.hd_exn Network.(seed Testnet)
    | false, None -> List.hd_exn Network.(seed Mainnet)
  in
  let port =
    match testnet, port with
    | _, Some port -> port
    | true, None -> Network.(port Testnet)
    | false, None -> Network.(port Mainnet)
  in
...
let command =
  let spec =
    ...
    +> flag
         ...
         ~doc:"dirname Data directory (data)"
    +> flag
         "-rundir"
         (optional_with_default "run" string)
         ~doc:"dirname Run directory (run)"
    +> flag
         "-logdir"
         (optional_with_default "log" string)
         ~doc:"dirname Log directory (log)"
