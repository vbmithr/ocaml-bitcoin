...
let bytes_with_msg ~len msg =
  ...
  Bytes.blit_string msg 0 buf 0 (min (Bytes.length buf - 1) (String.length msg));
...
module Timestamp = struct
  ...
  let of_int32_sec s = of_int_sec (Int32.to_int s)
  let to_int32_sec s = Int32.of_int (to_int_sec s)
  let of_int64_sec s = of_int_sec (Int64.to_int s)
  let to_int64_sec s = Int64.of_int (to_int_sec s)
...
module Hash (H2 : Digestif.S) (H1 : Digestif.S) = struct
  ...
  let of_string s =
    ...
    then invalid_arg (Printf.sprintf "Hash.of_string: length must be %d" length)
    else Hash s
  ;;

  let empty = of_string (String.make length '\x00')
  let of_hex_internal h = of_string (Hex.to_string h)
  let of_hex_rpc h = Hex.to_string h |> string_rev |> of_string
  ...
  let sexp_of_t t = Sexplib.Std.sexp_of_string (show t)
  let t_of_sexp sexp = of_hex_rpc (`Hex (Sexplib.Std.string_of_sexp sexp))

  let of_cstruct cs =
    Hash (Cstruct.to_string cs ~off:0 ~len:length), Cstruct.shift cs length
  ;;
  ...
  let compute_cstruct cs = compute_bigarray (Cstruct.to_bigarray cs)
  ...
  let compute_concat (Hash h1) (Hash h2) = compute_string (h1 ^ h2)
end
...
module BitcoinAddr = Base58.Bitcoin.Make (struct
    let sha256 x = Digestif.SHA256.(digest_string x |> to_raw_string)
  end)
...
module Chksum = struct
  ...
  let verify ~expected data = String.equal expected (compute data)

  exception Invalid_checksum of string * string

  let verify_exn ~expected data =
    let computed = compute data in
    if not (String.equal expected computed)
    then raise (Invalid_checksum (expected, computed))
  ;;
end
...
module ObjColl (C : COLL) = struct
  ...
  let to_cstruct cs objs ~f =
    ...
    let cs = CompactSize.to_cstruct_int cs len in
    C.fold_left objs ~init:cs ~f:(fun cs o -> f cs o)
  ;;
end
...
module Bitv = struct
  ...
  let t_of_sexp sexp = string_of_sexp sexp |> Bitv.L.of_string
  let sexp_of_t t = Bitv.L.to_string t |> sexp_of_string
  ...
  let to_string_le bitv =
    ...
    for i = 0 to nb_bytes - 1 do
      v := 0;
      for j = 0 to 7 do
        if Bitv.get bitv ((8 * i) + j) then v := !v lor (1 lsl j)
      done;
  ...
  let to_bool_list bv = Bitv.fold_right (fun v acc -> v :: acc) bv []
end
