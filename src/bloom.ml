...
let to_filter { filter; _ } =
  try Bitv.to_string_le filter with
  | _ -> invalid_arg "Bloom.to_string"
;;
...
let of_filter filter nb_funcs tweak =
  ...
  if len > bytes_max || nb_funcs > funcs_max then invalid_arg "Bloom.of_filter";
...
let create n p tweak =
  ...
  let filter_len_bytes =
    let open Float in
    min (-1. /. (log 2. *. log 2.) *. n *. log p /. 8.) (of_int bytes_max) |> to_int
  in
  let nb_funcs =
    let open Float in
    min (of_int filter_len_bytes *. 8. /. n *. log 2.) (of_int funcs_max) |> to_int
  in
...
let reset t = { t with filter = Bitv.(create (length t.filter) false) }
...
let _ =
  let data_hex =
    `Hex "019f5b01d4195ecbc9398fbf3c3b1fa9bb3183301d7a1fb3bd174fcfa40a2b65"
  in
