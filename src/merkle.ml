...
let compute a b =
  match a, b with
  | Node (_, h1, _), Node (_, h2, _) -> Hash256.compute_concat h1 h2
  | _ -> invalid_arg "Merkle.compute"
;;

let depth n =
  let rec inner acc n = if n = 0 then acc else inner (succ acc) (n / 2) in
...
let verify max_depth hashes flags =
  let rec inner depth hashes flags =
    ...
    | false :: flags, _, h :: hashes -> node Empty h Empty, hashes, flags
    ...
    | true :: flags, _, h :: hashes -> leaf h, hashes, flags
