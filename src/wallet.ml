...
module WIF = struct
  ...
  let of_base58 ctx { BitcoinAddr.version ; payload } =
    ...
    Key.read_sk ctx cs.buffer >>| fun privkey ->
...
module KeyPath = struct
  ...
  let to_string t =
    ListLabels.map t ~f:string_of_derivation |>
    String.concat "/"
  ...
  let write_be buf pos t =
    ...
    let len =
      ListLabels.fold_left t ~init:0 ~f:begin fun i v ->
  ...
  let write_be_cstruct cs t =
    let open Cstruct in
    ListLabels.fold_left t ~init:cs ~f:begin fun cs v ->
...
module Bip44 = struct
  module CoinType = struct
    ...
    let pp ppf ct =
      Format.fprintf ppf "%ld" (to_int32 ct)
  end
  ...
  module Chain = struct
    ...
    let pp ppf chain =
      Format.fprintf ppf "%ld" (to_int32 chain)
  end
  ...
  module Purpose = struct
    ...
    let pp ppf purpose =
      Format.fprintf ppf "%ld" (to_int32 purpose)
  end
